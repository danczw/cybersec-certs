...
function generateAllNotes(basenames) {
  ...
  typ += `#set page(paper: "a4", margin: (x: 14mm, top: 20mm, bottom: 16mm),\n`;
  typ += `  header: context {\n`;
  typ += `    let title = note-title.get()\n`;
  typ += `    let domain = note-domain.get()\n`;
  typ += `    let objective = note-objective.get()\n`;
  typ += `    if title != none and counter(page).get().first() > 1 [\n`;
  typ += `      #block(\n`;
  typ += `        width: 100%,\n`;
  typ += `        inset: (x: 5mm, y: 3mm),\n`;
  typ += `        radius: 2mm,\n`;
  typ += `        fill: gradient.linear(rgb("#667eea"), rgb("#764ba2"), angle: 135deg),\n`;
  typ += `      )[\n`;
  typ += `        #text(size: 12pt, weight: "bold", fill: white)[#title]\n`;
  typ += `        #h(1fr)\n`;
  typ += `        #text(size: 7.5pt, fill: white.transparentize(10%))[\n`;
  typ += `          #box(inset: (x: 1.5mm, y: 0.3mm), radius: 1.5mm, fill: white.transparentize(80%))[#domain]\n`;
  typ += `          #h(2mm)\n`;
  typ += `          #box(inset: (x: 1.5mm, y: 0.3mm), radius: 1.5mm, fill: white.transparentize(80%))[OBJ #objective]\n`;
  typ += `        ]\n`;
  typ += `      ]\n`;
  typ += `    ]\n`;
  typ += `  },\n`;
  ...
  typ += `      #v(1mm)\n`;
  typ += `      #text(size: 7pt, fill: luma(150))[Source: www.professormesser.com]\n`;
  typ += `      #h(1fr)\n`;
